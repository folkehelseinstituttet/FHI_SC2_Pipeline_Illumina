...
#PangoScrapper
try(content <- read_html("https://cov-lineages.org/lineage_list.html")) #Updated 04/08/2021
try(tables <- content %>% html_table(fill = TRUE))
try(pangolin.table<-tables[[1]])

warning.pango<-FALSE
if(!exists("pangolin.table")){ pangolin.table<-read.csv(pangolintable.path)
warning.pango<-TRUE
}

colnames(pangolin.table)<-tolower(colnames(pangolin.table))
